...
func cariIde(n *int, dataIde *kumpulanIde){
	...
	fmt.Scan(&data)
	fmt.Printf("Pilih cara search : \n1. Sequential Search\n2. Binary Search\nPilihan : ")
	fmt.Scan(&input)
	switch input {
		case 1 :
			sequentialSearch(n,dataIde,data)
		case 2 :
			binarySearch(n,dataIde,data)
	}
}
...
func sequentialSearch(n *int, dataIde *kumpulanIde, data string){
	...
	for i = 0 ; i < *n ; i++{
		if (dataIde[i].nama_ide == data ){
			...
			fmt.Printf("Dengan Skor Rating : %d",dataIde[i].rating)
		}
	}
	fmt.Println()
}
...
func binarySearch(n *int, dataIde *kumpulanIde, data string){
	var left, right, mid int
	
	left = 0
	right = *n-1
	mid = (left + right ) / 2
	for left <= right  && dataIde[mid].nama_ide != data {
		if (data <= dataIde[mid].nama_ide){
			right = mid - 1
		}else{
			left = mid + 1
		}
	...
	fmt.Printf("Nama Ide : %s\n", dataIde[mid].nama_ide)
	fmt.Printf("Dengan Skor Rating : %d",dataIde[mid].rating)
	fmt.Println()
}
